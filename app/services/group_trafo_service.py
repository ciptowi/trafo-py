...
def read_trafo_group_combobox(db: Session):
    groups = db.query(GroupTrafo).all()
    combobox = [GroupTrafoCombobox(id=group.id, name=group.name).model_dump() for group in groups]
